...
func (s *server) newClient(conn net.Conn) {
	log.Printf("New client joined: %s", conn.RemoteAddr().String())
...
func (s *server) nick(c *client, args []string) {
	c.nick = args[1]
	c.msg(fmt.Sprintf("Your nickname is set to %s", c.nick))
}
...
func (s *server) quitCurrentRoom(c *client) {
	if c.room != nil {
		...
		c.room.broadcast(c, fmt.Sprintf("User %s has left the chat\n", c.nick))
	}
}
